    ...
    def forward(
        ...
        if self.is_input_patches:  # here
            ...
            if self.adaln_single is not None:
                ...
                if is_fifo :
                    b = timestep.shape[0]
                    timestep = rearrange(timestep, 'b f -> (b f)').contiguous()
                timestep, embedded_timestep = self.adaln_single(
                    timestep, added_cond_kwargs, batch_size=batch_size, hidden_dtype=hidden_states.dtype
                ) # torch.Size([2f, 6912]) torch.Size([2f, 1152])

                if is_fifo:
                    timestep = rearrange(timestep, '(b f) d -> b f d', b=b).contiguous() 
                    embedded_timestep = rearrange(embedded_timestep, '(b f) d -> b f d', b=b).contiguous()
        ...
        # prepare timesteps for spatial and temporal block
        if is_fifo:
            timestep_spatial = rearrange(timestep, 'b f d -> (b f) d', b=input_batch_size).contiguous()  # torch.Size([34, 6912]
            timestep_temp = repeat(timestep, 'b f d -> (b p) f d', p=num_patches).contiguous() # torch.Size([2048, 17, 6912])
        else:
            timestep_spatial = repeat(timestep, 'b d -> (b f) d', f=frame + use_image_num).contiguous() # torch.Size([34, 6912]), f=17
            timestep_temp = repeat(timestep, 'b d -> (b p) d', p=num_patches).contiguous() # torch.Size([2048, 6912]), p=1024
        ...
        if self.is_input_patches:
            if self.config.norm_type != "ada_norm_single":
                ...
                hidden_states = self.norm_out(hidden_states) * (1 + scale[:, None]) + shift[:, None]
                hidden_states = self.proj_out_2(hidden_states)
            elif self.config.norm_type == "ada_norm_single": # True
                if is_fifo:
                    embedded_timestep = rearrange(embedded_timestep, 'b f d -> (b f) d').contiguous()
                else:
                    embedded_timestep = repeat(embedded_timestep, 'b d -> (b f) d', f=frame + use_image_num).contiguous()
                shift, scale = (self.scale_shift_table[None] + embedded_timestep[:, None]).chunk(2, dim=1)
                hidden_states = self.norm_out(hidden_states)
