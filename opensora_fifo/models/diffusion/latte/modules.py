...
class BasicTransformerBlock_(nn.Module):
    ...
    def forward(
        ...
        is_fifo = len(timestep.shape) == 3 # (b p) f 6d
        if is_fifo:
            f = timestep.shape[1]
        ...
        if self.use_ada_layer_norm:
            ...
            if is_fifo:
                shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
                        self.scale_shift_table[None, None] + timestep.reshape(batch_size, f, 6, -1)
                ).chunk(6, dim=2)

                shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = \
                    shift_msa.squeeze(2), scale_msa.squeeze(2), gate_msa.squeeze(2), shift_mlp.squeeze(2), scale_mlp.squeeze(2), gate_mlp.squeeze(2)
                # torch.Size([2048, 17, 1152]) * 6
            else:
                shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
                        self.scale_shift_table[None] + timestep.reshape(batch_size, 6, -1) # torch.Size([1, 6, 1152]) + torch.Size([2048, 6, 1152])
                ).chunk(6, dim=1) # torch.Size([2048, 1, 1152]) * 6
            norm_hidden_states = self.norm1(hidden_states) # torch.Size([2048, 17, 1152]) (same shape)
            norm_hidden_states = norm_hidden_states * (1 + scale_msa) + shift_msa # torch.Size([2048, 17, 1152]) (same shape)
            norm_hidden_states = norm_hidden_states.squeeze(1) # torch.Size([2048, 17, 1152]) (same shape)
        else:
